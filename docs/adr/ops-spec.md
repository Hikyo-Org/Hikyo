...
> **Declared amendment (2026-09-23, [#683](https://github.com/Hikyo-Org/Hikyo/issues/683), owner decision locked; cross-model review waived by the owner on 2026-09-23; operative upon the condition-reporting governance PR merging):** delivery-target report values: 100 target rows per service-account principal; report body ≤ 8 KiB; budget 12/min per principal and 300/min per org in a bucket separate from delivery fetches; staleness at `2 × resync_interval + 5 min` with `resync_interval` clamped to [30 s, 24 h]; reported-at future skew 5 min; purge after 30 days without a report. Catalogued in [ops-catalogue.md](../spec/ops-catalogue.md). This banner is a pointer; the normative contract is [k8s-condition-reporting.md](./k8s-condition-reporting.md). Historical text below is preserved.
