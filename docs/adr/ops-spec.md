...
> **Declared amendment (2026-09-25, [#806](https://github.com/Hikyo-Org/Hikyo/issues/806), per the [oss-mechanics.md](./oss-mechanics.md) amendment procedure; operative upon merge):** the uniform `429 + Retry-After` of § 4 and § 10 keeps one body and one status on every path, but `Retry-After` is now derived from the limiter that refused: a windowed limit (the § 4 per-IP 10/min and discovery allowances, the § 10 expensive-path rate budgets including `signup`, and the `mail-test` hour) advertises the whole seconds, rounded up, until its window admits that caller again absent new traffic. Refusals with no window (queue depth, concurrency caps, tracking saturation, the in-flight request cap) keep the fixed 5 s. The per-account backoff also keeps the fixed value by design: advertising its delay would disclose the presented account's consecutive-failure count to any prober. The header reflects only the caller's own history in the refusing window. Historical text below is preserved.

> **Declared amendment (2026-09-23, [#683](https://github.com/Hikyo-Org/Hikyo/issues/683), owner decision locked; cross-model review waived by the owner on 2026-09-23; operative upon the condition-reporting governance PR merging):** delivery-target report values (row quota, body size, a budget bucket separate from delivery fetches, heartbeat and staleness, future skew, purge, per-CR suppression) are new ops-spec entries; their values live only in [ops-catalogue.md](../spec/ops-catalogue.md) § Delivery-target condition reporting, so this banner cannot drift from them. This banner is a pointer; the normative contract is [k8s-condition-reporting.md](./k8s-condition-reporting.md). Historical text below is preserved.

> **Declared amendment (2026-09-06, user-approved Hikyo self-configuration):** The [approved self-configuration design](../spec/self-configuration-proposal.md) adds UI-driven activation without a process restart. One logical instance owns one protected configuration project. Its HA replicas share the datastore/root authority and one desired generation; independent remotes retain separate schemas, variables, histories and generations. The unified root screen groups owner-labelled references only. Its outage does not affect any remote's boot or runtime.
>
> Bounds are one unresolved apply per instance, ten new applies per minute, thirty seconds for preparation/convergence, and a two-second reconciliation cadence. Exact idempotent retries do not consume a new apply. Every live admitted HA node must prepare the candidate; an incompatible or absent participant blocks commitment. Participant membership is fixed per job. After commit, stale consumers must reconcile or refuse affected work even when reached directly, while administrative repair remains available. Post-commit timeout reports partial and keeps reconciling without automatic rollback. In-flight operations may finish using their captured immutable bundle.
>
> Mail setup/activation validates locally and never probes DNS or sends mail. Explicit tests retain the existing fifteen-second deadline, five/hour/principal and one concurrent/instance bounds, emit intent/outcome, and do not retry automatically. A failed relay does not undo configuration. The update channel affects release notifications only, never software installation. Existing installations explicitly adopt effective allowlisted values once; HA seed disagreement blocks adoption, and stale external managed inputs cannot override or invalidate the committed target afterward. Host-local status/recovery uses external database/root authority, requires quiesced consumers, fences competing writers and validates an exact uncollected revision. Recovery atomically aborts any stalled apply, releases its candidate retention root, and advances the target generation; an invalid desired revision cannot trap the operator behind an unresolved job. Restored managed credentials remain fenced until security reconciliation and explicit resumption; unlike process-owned SMTP, they are covered by backup and re-encryption.
...
> **Declared implementation extension (2026-09-14, PostgreSQL Kubernetes capacity):** the authoritative database-host monitor may be the kubelet serving the explicitly configured PostgreSQL node and PVC. Opt-in deployment wiring grants only node-scoped `get nodes/stats`, verifies the kubelet with the cluster CA, and uses a rotating ServiceAccount token. Samples must match the node, namespace and PVC, be at most two minutes old, and have valid capacity fields; unavailable or conflicting measurements remain unknown. The mapping is operator-asserted and must change with the database storage location. Measurements describe the backing filesystem, not the PVC allocation. Existing thresholds, admission and readiness remain unchanged.


> **Declared amendment (2026-09-05, [#638](https://github.com/Hikyo-Org/Hikyo/issues/638), operative upon this governance and retirement PR merging):** Populated-instance upgrades require a verified backup plus operator-held readable-restore evidence, superseding any loud-skip exception for upgrades. Maintenance is durable and fences writers. Automatic binary rollback requires proven pre-schema-write state; ambiguous or post-write recovery requires explicit operator restore and security reconciliation. The normative contract is [signed-upgrade-compatibility.md](./signed-upgrade-compatibility.md). Native Codex high-effort review concluded SOUND in three rounds; see [the review handoff](../handoff/638-signed-upgrade-governance.md). Historical text below is preserved.

> **Declared amendment (2026-09-04, [mcp-server.md](./mcp-server.md), [#627](https://github.com/Hikyo-Org/Hikyo/issues/627), per the [oss-mechanics.md](./oss-mechanics.md) amendment procedure; operative):** the composable-maxima catalogue gains the phase-1 MCP bounds: 256 KiB request body; 25 default and 100 maximum items per page; 256 KiB encoded `structuredContent`; 4 KiB compatibility text summary; 64 KiB discovery/catalog response; 2 KiB authenticated cursor with one non-renewing 15-minute chain validity; at most 10 pages, 1 000 items, and 1 MiB encoded structured content per chain; 30-second tool deadline; a per-service-account-principal token bucket refilling at 60 calls/minute with capacity 20; and concurrency 4/principal, 8/org, 64/instance. Page reads are datastore-bounded below transport and before collection materialization, each page reauthorizes, and page calls consume the same budgets. Rate/concurrency state and cursor authentication work across replicas and fail closed when their shared coordinator is unavailable. The MCP caps compose beneath §10's global HTTP limits; details and non-truncation behavior are fixed in the MCP ADR.
...
> **Declared amendment (2026-09-03, social sign-in and open registration, [#589](https://github.com/Hikyo-Org/Hikyo/issues/589) synthesis of [#578](https://github.com/Hikyo-Org/Hikyo/issues/578), per the [oss-mechanics.md](./oss-mechanics.md) amendment procedure; operative):** (a) the § 10 expensive-path budgets gain one named, rate-only, instance-wide category, **`signup`** (default 20/hour; the charge point is the first leg that would create state: the local sign-up request, or an unknown identity arriving on a `login` callback under intent `sign-up`; `none` and `fresh-org` landings share it; overflow is the uniform `429`); the pre-auth legs (federated callbacks, local submissions, verification) stay under § 4's admission and per-IP limits. (b) § 4's one-shot table gains the **sign-up verification token: 24 h**, single-use, hashed, fragment-carried, and the **establish window: 5 min**, unused-TTL of the session stamp an `establish` callback leaves (spent by the first mutation). (c) The composable-maxima catalogue gains the mailer entries: per-send deadline **15 s**; the operator **test send** under its own budget category `mail-test` (**5/hour per principal, 1 concurrent per instance**; instance scope, audited as a mail intent and outcome); the pending sign-up reaper on the existing hourly pruning cadence; and the per-policy **fresh-org cap, default 100**. (d) The Decision inventory gains row 21, *Open registration and social sign-in*. (e) The § 5 line "SMTP transport off by default" now points at a real mailer: process configuration under `HIKYO_MAIL_*` (address, TLS mode, CA file, user, password file or env as the documented weakest tier with exactly one source, `From`, EHLO name, allowed CIDRs), validated by name at boot and never dialed; nothing joins `reencrypt` or the restore re-entry checklist. (f) Operator runbook entries: Microsoft Entra rows are tenant-specific GUID issuers behind one multi-tenant app registration whose redirect-URI cap (256, or 100 with personal accounts) bounds the tenant count, with the `email` scope on the row and the `email` + `xms_edov` optional claims enabled (plus `auth_time` + `amr` for a reauth-capable row); never delete and recreate the Entra app (pairwise subjects); Google rows need the `email` scope, an HTTPS public-suffix redirect host (no IP literals, a tighter rule than the external-origin validator), and Google deletes idle clients after six months; GitHub apps need only `user:email`, and GitHub Enterprise Server documents no PKCE support. Values are catalogued in [ops-catalogue.md](../spec/ops-catalogue.md) and detailed in [social-signin.md](../spec/social-signin.md). Pre-existing discrepancy recorded, not fixed here: § 4 lists the credential-reset lifetime as 1 h while the code's shared constant is 24 h (invitation and sign-up tokens reuse it); reconciling the row is an editorial amendment of this ADR on its own ticket.
...
- **Metrics cardinality: no per-key, per-principal, per-env, or per-org/project *identity* labels; aggregate totals only** (a label value is a name leaked into the metrics store; #24's trust-boundary logic applies to Prometheus too). MCP uses only the closed methods `server/discover`, `tools/list`, `tools/call`, and `other`, plus its five registered tool names, `none`, and `other`; no request-controlled value becomes a label. With no dynamic identity labels the series set is static: **CI checks the registered families against the ≤ 1 000 budget and greps the forbidden label keys**; runtime cardinality cannot then exceed registration.
...
> **Amended 2026-09-01 ([#146](https://github.com/Hikyo-Org/Hikyo/issues/146) reopened per the [oss-mechanics](./oss-mechanics.md) governance procedure; operative):** the "HA: none in v1" posture below is promoted. Application-tier HA is now in scope as an opt-in mode (`HIKYO_HA=true`), gated on the scale-out trigger this spec recorded (post-1.0 demand, single-region HA first). The mode requires an externally highly available PostgreSQL datastore, stable unique node identities (`HIKYO_NODE_ID`), and a single shared root-key authority; SQLite stays single-node and refuses HA at boot, `--dev` root-key auto-generation is refused under HA, and a node whose root-key fingerprint or binary schema range disagrees with the datastore refuses to serve rather than degrade to a single-node fallback. Migrations serialize under goose's own PostgreSQL session lock (concurrent migrators wait, not race), and singleton background work (retention, scheduled backups) runs under a fenced datastore lease (`singleton_leases`, monotonic fence token) so it executes at most once and a stale leader cannot continue after losing authority; pre-authentication admission counters move to a shared table (`admission_counters`) so node hopping cannot bypass a per-IP, per-account, or per-issuer rate limit, while the per-node concurrency semaphore stays each node's own RAM budget. Sessions, CSRF, and every ceremony are already datastore-backed, so the load balancer needs no session stickiness; its health target is `/readyz`, which fails closed when the lease datastore is unreachable. Declared RTO = lease TTL + load-balancer probe period (lease 30 s, heartbeat 10 s). Advisory/SSE fan-out stays per node under HA, so a cross-node advisory falls back to revision polling bounded by the poll interval; the per-node JWKS cache stays bounded by its existing freshness TTL. `instance_identity` remains one row shared by every node: the nodes are one installation, not two, so the multi-instance ADR's two-live-installations refusal does not fire between them. Health and metrics gain label-free gauges `hikyo_ha_is_leader`, `hikyo_ha_nodes_seen`, `hikyo_ha_lease_age_seconds` (no per-node labels, per the § 4 bounded-cardinality posture). Full mechanics and the operator runbook: [#146](https://github.com/Hikyo-Org/Hikyo/issues/146) and `docs/handoff/146-multi-node-ha.md`.
...
| 21 | Open registration and social sign-in | `signup` 20/h instance-wide, charged at the first state-creating leg · sign-up token 24 h · establish window 5 min · mail send deadline 15 s · test send own admission class · fresh-org cap 100/policy · reaper hourly |
