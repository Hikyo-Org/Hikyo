...
> **Declared amendment (2026-09-14, [#742](https://github.com/Hikyo-Org/Hikyo/issues/742), operative upon owner decision, cross-model review SOUND, and the write-capability governance PR merging, per the [oss-mechanics.md](./oss-mechanics.md) amendment procedure):** the phase-boundary "any mutation, validation-as-dry-run, stage, publish, approval, or adapter action is out; separate ADR required" line is answered by [mcp-write.md](./mcp-write.md), which scopes a single non-secret configuration mutation. A write tool maps 1:1 to an existing events-emitting authorization operation (`value.stage`/`value.publish`), never relaxes the audit disposition, and is installed into the frozen registry only when a second operator flag `HIKYO_MCP_WRITE_ENABLED` is set on top of `HIKYO_MCP_ENABLED`. Registration-time exclusion, not a runtime check. Secret entry and reveal stay out and need their own amendment. This banner is a pointer; the normative contract is [mcp-write.md](./mcp-write.md), currently DRAFT. Historical phase-1 text below is preserved and remains read-only until this amendment is operative.
