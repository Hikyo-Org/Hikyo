...
> **Declared amendment (2026-09-19, [#781](https://github.com/Hikyo-Org/Hikyo/issues/781)):** `/mcp/codex` adds an explicit, stateless Streamable HTTP compatibility profile for native Codex's `2025-06-18` lifecycle. It accepts `initialize`, `notifications/initialized`, `notifications/cancelled`, `ping`, `tools/list`, and `tools/call`. Initialization requires that pinned version, client information and capabilities; unsupported versions receive endpoint guidance. No session is allocated or trusted. Notifications return empty 202 responses; cancellation follows the HTTP request context, with no detached work. The adapter supplies modern protocol metadata and mirror headers only inside this route, then sends tool requests through the existing `/mcp` handler and frozen registry. Caller capabilities never become authority. Host/origin checks, 256 KiB request bound, response bounds, pre-auth discovery admission, call admission, bearer checks, scope, secret policy and `origin=mcp` remain unchanged. Both endpoints share concurrency slots, feature gates and runtime readiness. JSON results and errors preserve request IDs and existing HTTP authentication/rate-limit dispositions. This explicitly amends the single-URI and modern-only clauses below; `/mcp` keeps its existing conformance contract. No new grant, credential class, publish, Apply, reveal or administrative operation is introduced. Native-client and both-profile domain tests accompany the amendment.

> **Declared amendment (2026-09-14, [#742](https://github.com/Hikyo-Org/Hikyo/issues/742), owner decision locked; cross-model review waived by the owner on 2026-09-17; operative upon the write-capability governance PR merging):** the phase-boundary "any mutation, validation-as-dry-run, stage, publish, approval, or adapter action is out; separate ADR required" line is answered by [mcp-write.md](./mcp-write.md), which scopes a stage-and-validate write surface. Two tools land: `hikyo_stage_change` maps 1:1 to the existing events-emitting `value.stage` op; `hikyo_validate_change` maps to a net-new non-mutating but audited `value.validate` op (new event `EventValueChangeValidated`). Neither relaxes the audit disposition; a write-surface tool is never `audited:none`. Both are installed into the frozen registry only when a second operator flag `HIKYO_MCP_WRITE_ENABLED` is set on top of `HIKYO_MCP_ENABLED` (registration-time exclusion, not a runtime check). The enforceable boundary is stage-and-validate never publish or deliver; publish, secret reveal, and any dedicated secret-input path stay out and need their own amendment. This banner is a pointer; the normative contract is [mcp-write.md](./mcp-write.md). Historical phase-1 text below is preserved and remains read-only until this amendment is operative.
