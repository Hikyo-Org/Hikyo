...
> **Declared amendment (2026-09-23, [#683](https://github.com/Hikyo-Org/Hikyo/issues/683), owner decision locked; cross-model review waived by the owner on 2026-09-23; operative upon the condition-reporting governance PR merging):** delivery-target status reports are attributed to the **service-account principal**, never to a credential id, so overlap rotation keeps one report row; principal deletion deletes its rows; a principal that no longer holds `report-delivery-status` shows its rows as reporter-revoked, never healthy. This banner is a pointer; the normative contract is [k8s-condition-reporting.md](./k8s-condition-reporting.md). Historical text below is preserved.
