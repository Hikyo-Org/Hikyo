...
function decisionHTML(decision) {
  ...
  return `<details class="decision" id="${decision.id}" data-group="${escape(decision.group)}" data-approval="${decision.approval}"${explicit ? ' open' : ''}>
    ...
    <div class="decision-body">
      ...
      <h4>Repository evidence</h4><div class="evidence-links">${decision.evidence.map((path) => sourceLink(path)).join('')}${link('#' + decision.id, 'Link to ' + decision.id)}</div>
    </div>
  </details>`;
}
...
<div class="shell">
  ...
  <main class="document" id="overview">
    ...
    <header class="hero">
      ...
      ${data.pendingClarification ? `<div class="status-note" id="bootstrap-clarification"><div><strong>One clarification pending</strong><h3>${escape(data.pendingClarification.question)}</h3><p>${escape(data.pendingClarification.reason)}</p><ul>${data.pendingClarification.options.map(([title, detail]) => `<li><strong>${escape(title)}</strong>: ${escape(detail)}</li>`).join('')}</ul><p><strong>Recommendation:</strong> ${escape(data.pendingClarification.recommendation)}</p><p>${escape(data.pendingClarification.status)}</p></div></div>` : ''}
    </header>
    ...
    <section class="section" id="catalogue" aria-labelledby="catalogue-title">
      <div class="section-top"><span class="section-number">03</span><h2 id="catalogue-title">What Hikyo will manage</h2></div>
      <p class="section-lede">D11 now requires every supported Hikyo server variable. The table below records the nine keys implemented in PR #686 before this scope expansion. The remaining families require additional activation work; their earlier external-only classification is superseded. Each remote owner validates its own schema and values.</p>
    ...
    <section class="section" id="activation" aria-labelledby="activation-title">
      ...
      <div class="simulator">
        ...
        <div class="sim-state" role="status" aria-live="polite" aria-atomic="true">
          <div class="sim-stats"><div class="sim-stat"><span>Durable target</span><strong id="desired-revision">r12</strong></div><div class="sim-stat"><span>Node A / Node B</span><strong id="node-revisions">r12 / r12</strong></div></div>
    ...
    <section class="section" id="delivery" aria-labelledby="delivery-title">
      ...
      <p class="section-lede">The earlier nine-key design is implemented in this branch. The user has expanded D11. The evidence and completed milestones below apply to the earlier implementation; they do not prove the new all-variable scope.</p>
      <div class="table-wrap"><table><caption>Implementation evidence</caption><thead><tr><th scope="col">Check</th><th scope="col">Current result</th></tr></thead><tbody>${(data.deliveryEvidence ?? []).map(([check, result]) => `<tr><td>${escape(check)}</td><td>${escape(result)}</td></tr>`).join('')}</tbody></table></div>
      ...
      <div class="sources">
        ...
        <div class="source"><div>${link('https://github.com/Hikyo-Org/Hikyo/issues/608', 'Mailer implementation #608')} · ${sourceLink('docs/spec/social-signin.md', 'Mailer specification')}</div><p>Issue remains open at investigation time. No live email reconfiguration is claimed in this report.</p></div>
...
console.log(`Built index.html: ${data.decisions.length} decisions, ${data.catalogue.length} managed keys, ${Buffer.byteLength(html)} bytes.`);
