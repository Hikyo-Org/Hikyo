...
function decisionHTML(decision) {
  ...
  return `<details class="decision" id="${decision.id}" data-group="${escape(decision.group)}" data-approval="${decision.approval}"${explicit ? ' open' : ''}>
    ...
    <div class="decision-body">
      ...
      <h4>${decision.evidenceScope === 'worktree' ? 'Current uncommitted worktree evidence' : 'Historical repository evidence'}</h4><div class="evidence-links">${decision.evidence.map((path) => decision.evidenceScope === 'worktree' ? `<code>${escape(path)}</code>` : sourceLink(path)).join('')}${link('#' + decision.id, 'Link to ' + decision.id)}</div>
    </div>
  </details>`;
}
...
<div class="shell">
  ...
  <main class="document" id="overview">
    ...
    <section class="section" id="catalogue" aria-labelledby="catalogue-title">
      ...
      <p class="section-lede">The current uncommitted catalogue has 27 top-level keys: nine historical mail/channel keys, 16 shared owner settings, secret node overrides and external bootstrap aliases. Ordinary owner and node consumers reload live. The complete-server-variable objective still has the explicit deployment gaps listed below.</p>
    ...
    <section class="section" id="delivery" aria-labelledby="delivery-title">
      ...
      <p class="section-lede">The current expansion has local implementation evidence. It is uncommitted and not exact-head CI verified. The completed milestone history and historical screenshots belong to the earlier nine-key implementation. New desktop evidence below exercises the expanded catalogue and prepare-before-MFA UI.</p>
