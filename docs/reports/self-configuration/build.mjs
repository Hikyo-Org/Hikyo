...
import coverage from './server-variable-coverage.json' with { type: 'json' };

const serverInputs = inventory.filter((entry) => entry.audience === 'server').map((entry) => entry.key).sort();
const mappedInputs = coverage.rows.map((entry) => entry.input).sort();
if (JSON.stringify(serverInputs) !== JSON.stringify(mappedInputs)) throw new Error('Server coverage must map every recognized server input exactly once.');
...
<div class="shell">
  ...
  <main class="document" id="overview">
    ...
    <section class="section" id="catalogue" aria-labelledby="catalogue-title">
      ...
      <p class="section-lede">The current catalogue has 27 top-level keys: nine mail/channel keys, 16 shared owner settings, secret node overrides and external bootstrap aliases. All 53 server inputs have managed consumers. Ordinary owner and node settings reload live; bootstrap settings use the supported deployment provider described below.</p>
      ...
      <h3 class="subheading">Server input to managed setting</h3>
      <p class="section-lede">${escape(coverage.verdict)}</p>
      <details class="external" id="server-coverage"><summary>Read the ${coverage.rows.length}-input source audit</summary>
      <p>${escape(coverage.shared_authority)} This source audit covers <code>${escape(coverage.baseline.slice(0, 8))}</code>. Runtime test evidence is recorded separately below.</p>
      <div class="table-wrap" tabindex="0" role="region" aria-label="Server input to managed setting; scroll horizontally on narrow screens"><table><caption>Every server input mapped to its managed representation and actual consumer.</caption><thead><tr><th scope="col">Server input</th><th scope="col">Managed setting</th><th scope="col">Activation and source evidence</th></tr></thead><tbody>${coverage.rows.map((entry) => `<tr><td><code class="key">${escape(entry.input)}</code><span class="key-note">${escape(entry.status)}</span></td><td><code class="key">${escape(entry.managed_key)}</code><span class="key-note">${escape(entry.boundary)}</span></td><td>${escape(entry.activation)}${entry.consumer_paths.map((path) => `<span class="key-note">${escape(path)}</span>`).join('')}</td></tr>`).join('')}</tbody></table></div>
      <p>${coverage.limits.map(escape).join(' ')}</p></details>
      <h3 class="subheading">Every recognized input</h3>
      <p class="section-lede">${inventory.length} recognized environment inputs, generated from the code inventory. The ${coverage.rows.length} server inputs have consumer mappings above. The remaining inputs belong to clients, operator commands or a retired feature. Paths identify sources; they are not permission to read arbitrary remote files.</p>
      <div class="table-wrap" tabindex="0" role="region" aria-label="Complete variable inventory; scroll horizontally on narrow screens"><table><caption>Recognized environment inputs and their scope. Client credentials are not imported into server configuration.</caption><thead><tr><th scope="col">Input</th><th scope="col">Consumer / scope</th><th scope="col">Required lifecycle</th><th scope="col">Source boundary</th></tr></thead><tbody>${inventory.map((entry) => `<tr><td><code class="key">${escape(entry.key)}</code>${entry.secret ? '<span class="key-note">Credential-bearing value</span>' : ''}${entry.referencedContentSecret ? '<span class="key-note">Referenced file contents are secret</span>' : ''}</td><td>${escape(entry.audience)} / ${escape(entry.scope)}</td><td>${escape(entry.activation)}</td><td>${escape(entry.import)}${entry.fileContentKey ? `<span class="key-note">Imports into ${escape(entry.fileContentKey)}</span>` : ''}</td></tr>`).join('')}</tbody></table></div>
    ...
    <section class="section" id="delivery" aria-labelledby="delivery-title">
      ...
      <p class="section-lede">The implementation is saved in signed checkpoints on draft PR #686. The table records current validation and remaining checks. Earlier milestone history is retained separately. Current desktop and mobile evidence exercises the expanded catalogue and prepare-before-MFA UI.</p>
      ...
      <p class="caption">Fresh complete Kubernetes proof: <a href="./validation/controlled-rollout-expanded-kind.json">sanitized runtime evidence</a> · <a href="./validation/controlled-rollout-expanded-source-manifest.json">795-file source manifest</a>. This uninterrupted run covers signed runtime source <code>c40833b9</code> and finishes at generation 19.</p>
      ...
      <h3 class="subheading">Deployment boundaries and proof limits</h3><ul class="acceptance-gaps">${data.acceptanceGaps.map((gap) => `<li>${escape(gap)}</li>`).join('')}</ul>
