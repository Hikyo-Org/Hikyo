...
| F03 release failure reported as call failure | `withAdmission`: the call's outcome wins; a release failure after success is recorded on the call state and logged by the transport (`Options.Log`, default `slog.Default()`), never joined into the result. | `TestReleaseFailureKeepsCommittedStageResult` (fake success + failed release: committed result returned, staging invoked once, warning logged without request material). `TestSharedAdmissionReleaseFailureKeepsTheResult` replaces the fail-closed read test; acquisition still fails closed (`TestSharedAdmissionLimitUsesUniformHTTP429`). |
