...
| Social sign-in and open registration (sixth family, locked 2026-09-03): **staged entry** (pick a method, then its form; one row per provider, one Microsoft row per Entra tenant; the "Create an account" door lists only policy-admitted methods and is absent while closed), sign-up confirmation step, check-your-mail, fragment-driven verification with `org_name` only on a fresh-org hint, the establish page as the invitation-claim surface, step-up refusal prompts, the Open registration panel and editor on Members per scope (inactive-with-cause, `n / cap`), Account & security for social-only accounts (Set vs Change password, "Verify with <provider>" establish branch), instance mailer line + test send, org list by origin, org rename. Provider brand rules per [social-providers.md](../research/social-providers.md). Public login page says only "Sign-up is paused." | `docs/site/public/prototypes/social-signin/2/` (served at `/prototypes/social-signin/2/`); iteration 1 kept for the discarded variants | [#587](https://github.com/Hikyo-Org/Hikyo/issues/587) |

**Handoff note for the social sign-in family** ([#587](https://github.com/Hikyo-Org/Hikyo/issues/587), [#604](https://github.com/Hikyo-Org/Hikyo/issues/604)): "last used method first, rest collapsed" on the staged step 1 is a client-side candidate for an implementation-time polish, not a prototype re-iteration; the login page under a CLI login handoff or device-code approval hides the sign-up door and sends intent `sign-in`; the `--provider <kind>:<slug>` CLI preselect is a query hint on the login page that only chooses step 2. Semantics live in [social-signin.md](./social-signin.md).
