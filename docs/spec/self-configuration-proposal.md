...
Status: design approved and implemented, 2026-09-06. Local validation passes and independent standards/spec reviews are clean. The owning ADRs contain explicit amendments; the [validation record](../reports/self-configuration/validation.md) distinguishes verified behavior from delivery steps still pending.
...
2. Use the existing matrix, drafts, publication, history and rollback. Instance settings links to the project. Saving a draft and publishing a revision do not activate it. A reauthenticated instance administrator applies an exact published revision through the UI.
3. Manage nine keys in the first delivery: full mail configuration, including a new managed-only `HIKYO_MAIL_CA_PEM`, plus `HIKYO_UPDATE_CHANNEL`. The report lists defaults, absence semantics, classification, validation, reload owners and all excluded current config families. DB/root-key bootstrap, authentication/networking, backup/audit policy and client-only settings retain existing authority.
...
| [D09](../reports/self-configuration/index.html#D09) | Should applying settings require reauthentication or another reviewer? | Fresh reauthentication, with no mandatory second person. | Delegated approval |
...
| [D11](../reports/self-configuration/index.html#D11) | Which configuration belongs inside Hikyo? | A closed catalogue, starting with the complete mail configuration and release-notification channel. | Delegated approval |
...
These are implementation requirements, not completed milestones. Consult the [handoff](../handoff/self-configuration-design.md) before implementation.
...
The HTML report is a working document artifact. Runtime behavior still requires implementation and verification.
