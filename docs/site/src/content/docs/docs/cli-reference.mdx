...
| `7` | Throttled (`429`) | Wait the time the error names, then retry. |

Code `5` deliberately combines missing and unauthorized resources.

Code `7` means that the server is up but refused the call because of a rate
limit or overload. The error line on stderr names the wait, for example
`hikyo: too many requests (retry after 40s)`. Scripts must branch on the exit
