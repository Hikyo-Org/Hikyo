...
Exit code `7` with `too many requests; Retry-After: N` means the server
throttled the caller. Wait `N` seconds before retrying. Repeated throttling of
CI jobs usually means they share one egress address and its per-IP allowance;
follow the shared-egress advice under
[Rate limits](/docs/cli-reference/#rate-limits).
