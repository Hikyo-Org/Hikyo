...
For cloud VMs, use the [deployment-specific upgrade guide](/docs/cloud-upgrades/).
For a fresh Docker nightly and its image replacement commands, use
[cloud nightlies](/docs/cloud-nightlies/).
...
## Social sign-in schema upgrade

The version that adds the social sign-in schema (migration 00057) closes every
open reauthentication window while it reshapes the sessions table. Sessions
survive, so nobody is signed out, but a user who had just reauthenticated for a
protected environment reauthenticates again. OIDC sign-ins in progress during
the migration are cancelled and simply restart. The contact email an account
had keeps its value, in canonical form (domain lowercased), when it is a valid
plain address, and is cleared otherwise; kept addresses are marked unverified
and are never used to sign in. Nothing else changes until registration is
enabled.
