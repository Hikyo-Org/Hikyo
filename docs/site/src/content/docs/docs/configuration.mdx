...
| `HIKYO_MCP_WRITE_ENABLED` | `false` | Installs the `hikyo_stage_change` and `hikyo_validate_change` tools. Requires `HIKYO_MCP_ENABLED=true`. Grants no authority: each call needs an `edit` grant on the environment and is audited. Nothing publishes or delivers over MCP. |
...
| `hikyo_admission_throttled_total` | counter | none | Pre-auth attempts the per-source-IP window refused on this node since boot. |
