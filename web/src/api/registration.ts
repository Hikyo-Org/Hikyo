...
import type {
  RegistrationExternalEntry,
  RegistrationPolicy,
  RegistrationPolicyPutRequest,
} from '@hikyo/client';
...
export function useRegistrationPolicy(
  ...
  enabled: boolean,
): UseQueryResult<RegistrationPolicy | null> {
  return useQuery({
...
export async function putRegistrationPolicy(
  ...
  return scope.kind === 'instance'
    ? parsed(putInstanceRegistrationPolicyOp, { body })
    : parsed(putOrgRegistrationPolicyOp, { path: { org: scope.org }, body });
}
...
export function resaveBody(policy: RegistrationPolicy): RegistrationPolicyPutRequest {
  return {
    external: policy.external.map((entry) => ({
      provider: entry.provider,
      ...(entry.claim === undefined ? {} : { claim: entry.claim, values: entry.values ?? [] }),
    })),
    ...(policy.local === undefined ? {} : { local: { domains: policy.local.domains ?? [] } }),
    landing: policy.landing,
  };
}

/** The requirement each admitted method places on a signer (#598). */
export function requirementLine(kind: string): string {
  switch (kind) {
    case 'oauth2':
      return 'GitHub must hold a primary, verified address.';
    case 'local':
...
export function providerLabel(entry: RegistrationExternalEntry): string {
  return entry.display_name ?? entry.provider.slug;
}
...
export function preconditionText(detail: string): string {
  ...
  if (detail.startsWith('provider-disabled')) {
    return `${slug} is disabled or no longer configured. Enable it, or leave it off.`;
  }
  return detail;
}

/** The remedy the panel shows for an inactive policy; the cause never reaches the public page. */
export function inactiveText(policy: RegistrationPolicy, authority: string): string {
  switch (policy.inactive_cause) {
    case 'authority-lost':
    ...
    case 'precondition':
      return `Sign-ups are refused: ${preconditionText(policy.inactive_precondition ?? 'a precondition failed')}`;
    default:
      return '';
  }
}
