...
export function useSelfConfigActions() {
  ...
  const apply = useMutation({ mutationFn: (body: { revision: bigint; expected_generation: bigint; schema_version: number; idempotency_key: string; confirm_restored_credentials: boolean }) => parsed(applyInstanceConfigOp, { body: { ...body, revision: wireInteger(body.revision), expected_generation: wireInteger(body.expected_generation) }, ...transport }), onSettled: refresh });
