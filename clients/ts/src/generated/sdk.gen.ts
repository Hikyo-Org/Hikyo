...
/**
 * Public runtime availability for maintenance and reconnection.
 *
 * Reports only this instance's availability and upgrade phase. No tenant
 * data, identity, configuration, paths, or failure details are exposed.
 * Polling does not extend sessions. Storage failures return unavailable,
 * never a guessed ready or maintenance state.
 *
 */
export const getRuntimeStatus = <ThrowOnError extends boolean = false>(options?: Options<GetRuntimeStatusData, ThrowOnError>) => (options?.client ?? client).get<GetRuntimeStatusResponses, GetRuntimeStatusErrors, ThrowOnError>({ url: '/api/v1/runtime/status', ...options });
...
/**
 * Satisfy a login challenge with a TOTP code; mints the session.
 *
 * Consumes the single-use login challenge (single-use, expiring, bound to
 * the account) and mints the browser session the login asked for, with
 * `totp` recorded alongside `password` in the assurance. Refusals mirror
 * the step-up path: `401` for a wrong code, `409` for a code already used
 * for its time step, `429` for the factor budget.
 *
 */
export const loginChallengeTotp = <ThrowOnError extends boolean = false>(options: Options<LoginChallengeTotpData, ThrowOnError>) => (options.client ?? client).post<LoginChallengeTotpResponses, LoginChallengeTotpErrors, ThrowOnError>({
...
/**
 * Begin a passkey assertion against a login challenge.
 *
 * Resolves the login challenge to its account and opens a WebAuthn
 * assertion ceremony for that account's registered passkeys. Mints
 * nothing; the ceremony is completed at `.../webauthn/finish`.
 *
 */
export const loginChallengeWebauthnStart = <ThrowOnError extends boolean = false>(options: Options<LoginChallengeWebauthnStartData, ThrowOnError>) => (options.client ?? client).post<LoginChallengeWebauthnStartResponses, LoginChallengeWebauthnStartErrors, ThrowOnError>({ url: '/api/v1/auth/login/challenge/{challenge}/webauthn/start', ...options });

/**
 * Complete a passkey assertion against a login challenge; mints the session.
 *
 * Validates the assertion against the live ceremony, consumes the login
 * challenge, and mints the browser session with `webauthn` recorded
 * alongside `password` in the assurance. A signature-count regression
 * disables the cloned credential (audited) and refuses.
 *
 */
export const loginChallengeWebauthnFinish = <ThrowOnError extends boolean = false>(options: Options<LoginChallengeWebauthnFinishData, ThrowOnError>) => (options.client ?? client).post<LoginChallengeWebauthnFinishResponses, LoginChallengeWebauthnFinishErrors, ThrowOnError>({
 ...
 * Automation requires project-scoped definitions-edit and cannot delete
 * a protected environment. Deletion removes values, drafts, pins and
 * revision payloads; audit events remain subject to audit retention.
 *
 */
...
/**
 * Close registration at this scope.
 *
 * Deletes the policy and, in the same transaction, the pending local
 * sign-ups it admitted (each audited `registration.signup_expired`,
 * cause `policy-deleted`). Orgs the policy minted keep their origin and
 * policy pointer. Reauthentication-gated like `put`; no session is
 * purged.
 *
 */
export const deleteInstanceRegistrationPolicy = <ThrowOnError extends boolean = false>(options: Options<DeleteInstanceRegistrationPolicyData, ThrowOnError>) => (options.client ?? client).delete<DeleteInstanceRegistrationPolicyResponses, DeleteInstanceRegistrationPolicyErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Read the instance registration policy.
 *
 * The instance's open-registration policy (#579), or `404` when the
 * scope has none: registration is closed. The standing delegation and
 * the preconditions are re-checked on this read, so `state` is live: an
 * authority principal who no longer holds the landing's grants, or a
 * failing precondition, reads `inactive` with its cause. Recorded as a
 * membership-surface read.
 *
 */
export const getInstanceRegistrationPolicy = <ThrowOnError extends boolean = false>(options?: Options<GetInstanceRegistrationPolicyData, ThrowOnError>) => (options?.client ?? client).get<GetInstanceRegistrationPolicyResponses, GetInstanceRegistrationPolicyErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/v1/instance/registration-policy',
    ...options
});
...
/**
 * Create or replace the instance registration policy.
 *
 * A full replacement; the caller becomes the authority principal (any
 * edit reassigns it, #579 d4). Reauthentication-gated: `proof` carries a
 * TOTP code, or the password where no factor is enrolled, consumed in
 ...
 *
 */
export const putInstanceRegistrationPolicy = <ThrowOnError extends boolean = false>(options: Options<PutInstanceRegistrationPolicyData, ThrowOnError>) => (options.client ?? client).put<PutInstanceRegistrationPolicyResponses, PutInstanceRegistrationPolicyErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/v1/instance/registration-policy',
    ...options,
    headers: {
 ...
 *
 */
export const deleteOrgRegistrationPolicy = <ThrowOnError extends boolean = false>(options: Options<DeleteOrgRegistrationPolicyData, ThrowOnError>) => (options.client ?? client).delete<DeleteOrgRegistrationPolicyResponses, DeleteOrgRegistrationPolicyErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Read the organisation registration policy.
 *
 * The organisation's open-registration policy (#579), or `404` when the
 * scope has none: registration is closed. The standing delegation and
 * the preconditions are re-checked on this read, so `state` is live: an
 * authority principal who no longer holds the landing's grants, or a
 * failing precondition, reads `inactive` with its cause. Recorded as a
 ...
 *
 */
export const getOrgRegistrationPolicy = <ThrowOnError extends boolean = false>(options: Options<GetOrgRegistrationPolicyData, ThrowOnError>) => (options.client ?? client).get<GetOrgRegistrationPolicyResponses, GetOrgRegistrationPolicyErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/v1/orgs/{org}/registration-policy',
    ...options
});
...
/**
 * Create or replace the organisation registration policy.
 *
 * A full replacement; the caller becomes the authority principal (any
 * edit reassigns it, #579 d4). Reauthentication-gated: `proof` carries a
 * TOTP code, or the password where no factor is enrolled, consumed in
 ...
 *
 */
export const putOrgRegistrationPolicy = <ThrowOnError extends boolean = false>(options: Options<PutOrgRegistrationPolicyData, ThrowOnError>) => (options.client ?? client).put<PutOrgRegistrationPolicyResponses, PutOrgRegistrationPolicyErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/v1/orgs/{org}/registration-policy',
    ...options,
    headers: {
 ...
 *
 * Updates the username and display name. The email is not profile data and cannot be set here; only verified local sign-up writes a verified one. Provisioned names remain controlled by SCIM. Username changes require current account proof.
 */
...
/**
 * Read this owner instance configuration status.
 *
 * Human administration on the owning instance. Remote workspaces call the
 * owner directly. Instance authority never substitutes for bound project grants.
 *
 */
export const getInstanceConfig = <ThrowOnError extends boolean = false>(options?: Options<GetInstanceConfigData, ThrowOnError>) => (options?.client ?? client).get<GetInstanceConfigResponses, GetInstanceConfigErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Preview one-time adoption without disclosing values.
 *
 * Human administration on the owning instance. Remote workspaces call the
 * owner directly. Instance authority never substitutes for bound project grants.
 *
 */
export const previewInstanceConfigAdoption = <ThrowOnError extends boolean = false>(options?: Options<PreviewInstanceConfigAdoptionData, ThrowOnError>) => (options?.client ?? client).get<PreviewInstanceConfigAdoptionResponses, PreviewInstanceConfigAdoptionErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Adopt the exact previewed effective configuration.
 *
 * Human administration on the owning instance. Remote workspaces call the
 * owner directly. Instance authority never substitutes for bound project grants.
 *
 */
export const adoptInstanceConfig = <ThrowOnError extends boolean = false>(options: Options<AdoptInstanceConfigData, ThrowOnError>) => (options.client ?? client).post<AdoptInstanceConfigResponses, AdoptInstanceConfigErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/v1/instance/config/adoption',
    ...options,
    headers: {
...
/**
 * Apply one published revision on this logical instance.
 *
 * Human administration on the owning instance. Remote workspaces call the
 * owner directly. Instance authority never substitutes for bound project grants.
 *
 */
export const applyInstanceConfig = <ThrowOnError extends boolean = false>(options: Options<ApplyInstanceConfigData, ThrowOnError>) => (options.client ?? client).post<ApplyInstanceConfigResponses, ApplyInstanceConfigErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Send one test message using exactly the selected revision.
 *
 * Human administration on the owning instance. Remote workspaces call the
 * owner directly. Instance authority never substitutes for bound project grants.
 *
 */
export const testInstanceConfigMail = <ThrowOnError extends boolean = false>(options: Options<TestInstanceConfigMailData, ThrowOnError>) => (options.client ?? client).post<TestInstanceConfigMailResponses, TestInstanceConfigMailErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * List declarations used by the next publication.
 */
export const listEnvironmentParameters = <ThrowOnError extends boolean = false>(options: Options<ListEnvironmentParametersData, ThrowOnError>) => (options.client ?? client).get<ListEnvironmentParametersResponses, ListEnvironmentParametersErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Add or delete one parameter declaration for the next publication.
 */
export const changeEnvironmentParameter = <ThrowOnError extends boolean = false>(options: Options<ChangeEnvironmentParameterData, ThrowOnError>) => (options.client ?? client).post<ChangeEnvironmentParameterResponses, ChangeEnvironmentParameterErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/v1/orgs/{org}/projects/{project}/environments/{environment}/parameters',
    ...options,
    headers: {
...
/**
 * List the environment's delivery targets and their derived state.
 *
 * Two labelled layers, never merged into one health bit (ADR D2): per
 * reporting principal, the server's own observation (its last
 * authenticated delivery fetch here) and its quota notice; per target,
 * the conditions its controller asserted, with a state derived at read
 * time. `reporter-revoked`, `refused` and `stale` take precedence over
 * `reported`, in that order; nothing is stored as healthy. An
 * environment the caller cannot read is the uniform nonexistent answer,
 * never an empty or redacted list.
 *
 */
export const listDeliveryTargets = <ThrowOnError extends boolean = false>(options: Options<ListDeliveryTargetsData, ThrowOnError>) => (options.client ?? client).get<ListDeliveryTargetsResponses, ListDeliveryTargetsErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Report one delivery target's asserted conditions.
 *
 * Upserts the caller's latest-state row for one target, keyed by the
 * presenting principal (never the credential, so rotation keeps one row)
 * and the target's cluster, instance and object UIDs. An accepted repeat
 * report is not audited; a new row, and every refusal, is.
 *
 * Refusals: a caller without `report-delivery-status` on this
 * environment is the uniform 404; a value outside the advertised
 * vocabulary refuses the whole report with 422 naming the member and
 * stores none of it (only the closed cause, on an existing row); a lower
 * `observed_generation`, a not-later `reported_at` within one
 * generation, or a `reported_at` more than 5 minutes ahead is a 409 that
 * is audited and dropped; a principal already reporting 100 targets is
 * the named 409 `limit_exceeded`; a body over 8 KiB is a 413 that is
 * never tied to a row. Charged to a separate budget, 60/min per principal
 * and 300/min per organisation, after authorization.
 *
 */
export const reportDeliveryTarget = <ThrowOnError extends boolean = false>(options: Options<ReportDeliveryTargetData, ThrowOnError>) => (options.client ?? client).post<ReportDeliveryTargetResponses, ReportDeliveryTargetErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
    url: '/api/v1/orgs/{org}/projects/{project}/environments/{environment}/delivery-targets',
    ...options,
    headers: {
...
/**
 * Remove the caller's row for one deleted delivery target.
 *
 * The best-effort tombstone a controller sends when its target is
 * deleted (ADR D6): the row is removed right after the tombstone event.
 * A target the caller has no row for is the uniform 404.
 *
 */
export const tombstoneDeliveryTarget = <ThrowOnError extends boolean = false>(options: Options<TombstoneDeliveryTargetData, ThrowOnError>) => (options.client ?? client).post<TombstoneDeliveryTargetResponses, TombstoneDeliveryTargetErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Compare two revisions with secret write-presence only.
 *
 * Both revisions must retain their payload. A secret on either side makes
 * the entire row secret. Secret status uses write-presence only. Explicit
 * per-key reveal checks current reveal or reveal-history for each side,
 * runs one key-bound ceremony, and audits each disclosed revision.
 *
 */
export const diffRevisions = <ThrowOnError extends boolean = false>(options: Options<DiffRevisionsData, ThrowOnError>) => (options.client ?? client).post<DiffRevisionsResponses, DiffRevisionsErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
...
/**
 * Reveal one key across two revisions.
 *
 * Both revisions must retain their payload. A secret on either side makes
 * the entire row secret. Secret status uses write-presence only. Explicit
 * per-key reveal checks current reveal or reveal-history for each side,
 ...
 *
 */
export const revealRevisionDiff = <ThrowOnError extends boolean = false>(options: Options<RevealRevisionDiffData, ThrowOnError>) => (options.client ?? client).post<RevealRevisionDiffResponses, RevealRevisionDiffErrors, ThrowOnError>({
    security: [{ scheme: 'bearer', type: 'http' }],
