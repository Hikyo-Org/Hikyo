...
func TestRedactTokens(t *testing.T) {
	...
	cases := []struct {
		...
		{"handoff state type", "hik_1_hs_" + strings.Repeat("h", 43), RedactionMarker},
		{"handoff code type", "hik_1_hc_" + strings.Repeat("c", 43), RedactionMarker},
		{"sign-up token type", "hik_1_su_" + strings.Repeat("s", 43), RedactionMarker},
		{"sign-up token in a fragment", "https://hikyo.test/signup#token=hik_1_su_" + strings.Repeat("s", 43) + "&landing=none", "https://hikyo.test/signup#token=" + RedactionMarker + "&landing=none"},
...
// TestScanningFindingSpecsAreClosed proves the four scanning.* finding events
// (#74, ADR section 5) are registered and that their closed schemas make
// matched content unrepresentable: no field name carries matched material, and
// validate() refuses a payload that smuggles it in (SS4). The per-field shapes
// are covered by the registry's own well-formedness validation.
func TestScanningFindingSpecsAreClosed(t *testing.T) {
	finding := []EventType{
	...
	for _, et := range finding {
		...
		for _, forbidden := range []string{"match", "matched_text", "offset", "length", "excerpt", "fingerprint", "value"} {
			if _, ok := spec.Schema[forbidden]; ok {
				t.Errorf("%s schema declares forbidden field %q", et, forbidden)
			}
		}
		if err := spec.Schema.validate(et, Payload{"rule_id": "x", "matched_text": "AKIA..."}); err == nil {
			t.Errorf("%s accepted a payload carrying matched_text — the closed schema must reject it", et)
		}
	}
}
