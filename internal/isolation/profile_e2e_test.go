...
func TestAccountProfileSelfService(t *testing.T) {
	forEngines(t, func(t *testing.T, db *store.DB) {
		...
		if before.Username != "profile-admin" || before.Email != nil || before.Managed {
		...
		execRaw(t, db, fmt.Sprintf("UPDATE accounts SET email = 'admin@example.test' WHERE id = '%s'", admin.accountID))
		email := "admin@example.test"
		next := service.ProfileUpdate{Username: "pretty-admin", DisplayName: "Pretty Admin"}
		want := service.AccountProfile{Username: next.Username, DisplayName: next.DisplayName, Email: &email, UsernameEditable: true}
		...
		same := func(got service.AccountProfile) bool {
			return got.Username == want.Username && got.DisplayName == want.DisplayName && got.Managed == want.Managed &&
				got.UsernameEditable == want.UsernameEditable && got.Email != nil && *got.Email == *want.Email
		}
		if !same(after) {
		...
		if got := queryInt(t, db, fmt.Sprintf("SELECT COUNT(*) FROM accounts WHERE id = '%s' AND email = 'admin@example.test'", admin.accountID)); got != 1 {
