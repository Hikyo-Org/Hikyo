...
func TestAuthenticatedBackupPreparationPinsFinalTargetAndRefusesMissingProof(t *testing.T) {
	for _, engine := range []releaseidentity.Engine{releaseidentity.SQLite, releaseidentity.Postgres} {
		for _, managed := range []bool{false, true} {
			t.Run(fmt.Sprintf("%s/managed=%t", engine, managed), func(t *testing.T) {
				...
				var unchanged func()
				if managed {
					unchanged = seedOldManagedPreview(t, cfg, booted, request.RootKey)
					request.CheckConfiguration = assertManagedPreview(t)
				}
				...
				request.CheckConfiguration = func(ctx context.Context, projection *upgrade.CandidateConfiguration, values map[string]string) error {
					called = true
					if managed {
						return assertManagedPreview(t)(ctx, projection, values)
					}
					return nil
				}
				...
				if _, err := invoke(request, 2); managed {
					if err != nil || !called {
						t.Fatalf("managed maintenance preview: called=%t err=%v", called, err)
					}
					unchanged()
