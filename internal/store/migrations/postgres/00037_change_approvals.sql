...
-- for secret and configuration CHANGES in sensitive scopes, promoted into 1.0
-- by the mvp-boundary declared amendment 3 (the recorded compliance trigger
-- fired). ASCII only, matching the sqlite twin.
--
-- The design reuses the locked draft/publish (#51), permission (#55),
-- reauthentication-ceremony (#58) and audit foundations. An approval authorises
-- ONE exact reviewed change set and is never a second mutation path: merge
-- rides the ordinary validated publish, and the request pins the existing
-- publish preview-token digest over the closed selection. Later edits, target
-- movement or a policy change invalidate the approval rather than inheriting
-- it. Everything here is installation tenant data under the ordinary chain, so
-- the scope classes are project (the policy and its child sets) and environment
-- (a request and its votes address one environment).
...
-- narrows it to that environment (folders are organisational only, so folder
-- scope is deliberately absent). No FK on environment_id: '' is a real value
...
-- version increments on every update so an open request pinned to an older
-- version fails closed when the policy moves.
...
-- reauthentication, a reason, and a high-signal audit event. Bypass cannot
-- silently weaken or mutate the policy.
