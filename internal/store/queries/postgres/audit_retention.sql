...
WHERE (audit_tenant_events.org_id, COALESCE(audit_tenant_events.correlation_id, audit_tenant_events.id)) IN (
    ...
    WHERE audit_tenant_events.recorded_at < CASE WHEN audit_tenant_events.type IN (SELECT jsonb_array_elements_text(sqlc.arg(access_types)::text::jsonb)) THEN sqlc.arg(access_cutoff_time)::timestamptz ELSE sqlc.arg(security_cutoff_time)::timestamptz END
      AND NOT EXISTS (
        ...
        WHERE sibling.org_id = audit_tenant_events.org_id AND COALESCE(sibling.correlation_id, sibling.id) = COALESCE(audit_tenant_events.correlation_id, audit_tenant_events.id)
          AND sibling.recorded_at >= CASE WHEN sibling.type IN (SELECT jsonb_array_elements_text(sqlc.arg(access_types)::text::jsonb)) THEN sqlc.arg(access_cutoff_time)::timestamptz ELSE sqlc.arg(security_cutoff_time)::timestamptz END
      )
...
WHERE COALESCE(audit_instance_events.correlation_id, audit_instance_events.id) IN (
    ...
    WHERE audit_instance_events.recorded_at < CASE WHEN audit_instance_events.type IN (SELECT jsonb_array_elements_text(sqlc.arg(access_types)::text::jsonb)) THEN sqlc.arg(access_cutoff_time)::timestamptz ELSE sqlc.arg(security_cutoff_time)::timestamptz END
      AND NOT EXISTS (
